...
lint:
	uv run ruff check .
	uv run ty check django_domain_events
...
# Release pipeline. The version lives in django_domain_events/version.py (pyproject
...
RELEASE_PACKAGE_NAME := django-domain-events
RELEASE_VERSION_FILES := django_domain_events/version.py|^__version__[^=]*= *
