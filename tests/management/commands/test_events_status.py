...
def test_it_names_the_receivers_that_are_behind(order: OrderPlaced, record: list[str]) -> None:
    ...
    assert "testapp.durable_receiver\towed=1\tdead=0" in output
...
def test_the_oldest_owed_is_reported_as_an_age(order: OrderPlaced, record: list[str]) -> None:
    ...
    with transaction.atomic():
        fire(order)
    DeliveryRecord.objects.update(available_at=datetime.now(timezone.utc) - timedelta(hours=2))
    assert "oldest owed   72" in _run() or "oldest owed   71" in _run()
...
def test_json_is_parseable_and_carries_the_same_numbers(
    ...
    assert {"key": "testapp.with_context", "owed": 0, "dead": 1} in parsed["receivers"]
